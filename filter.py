...
def is_valid_input(row):
    try:
        if int(row['custom_count']) < 0:
            raise ValueError(f"Custom count must be a non-negative integer: row: {row}")
    except:
            raise TypeError(f"Invalid value for custom count: row: {row}")
    try:
        if int(row['time_cost']) < 0:
            raise ValueError(f"Time cost must be a non-negative integer: row: {row}")
    except:
            raise TypeError(f"Invalid value for time cost: row: {row}")
    try:
...
def main():
    ...
    with open('input.csv', 'r') as file:
        ...
        with open('request.csv','w',newline='') as file:
            ...
            fieldnames = ['uid','custom_count', 'created_at', 'time_cost', 'table_type', 'vip']
