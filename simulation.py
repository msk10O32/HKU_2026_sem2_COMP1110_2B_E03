...
class Restaurant:
    def __init__(self,env,requests,groups):
        ...
        for group in groups:
            self.table_groups[group["type"]] = simpy.PriorityResource(env, capacity=int(group["group_capacity"]))
            self.queue_size[group["type"]] = float(group["queue_capacity"])
        self.env = env
    ...
    def handle_request(self,request):
        ...
        if len(self.table_groups[request['table_type']].queue) <= self.queue_size[request['table_type']]:
        ...
        with self.table_groups[request['table_type']].request(priority=-int(request['is_vip'])) as req:
            ...
            print(f"Request {request['uid']} type: {request['table_type']} is served at time {seating_time}")
