...
# -*- coding: utf-8 -*-
"""
Restaurant Queue Simulation - Metrics Calculator
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Calculate restaurant queue simulation metrics')
    ...
    parser.add_argument('--output', '-o', default=None,
                        help='Output report filename (default: interactively ask if not provided)')
...
def main():
    ...
    if args.output is not None:
        ...
        default_name = 'metrics_report.txt'
        default_dir = os.path.dirname(csv_path) or '.'
        default_full_path = os.path.join(default_dir, default_name)

        print(f"\nDefault output location: {default_full_path}")
        user_input = input("Enter output filename (or press Enter to use default): ").strip()

        if user_input == "":
            output_file = default_full_path
        else:
        ...
        if os.path.exists(output_file):
            ...
            if overwrite != 'y':
                print("Operation cancelled. Report not saved.")
                return
